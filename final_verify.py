...
def init_database():
    ...
    try:
        ...
        if all_exist:
            log(f"表列表：{tables}")
            save_result("数据库表结构", True, f"4 个核心表已创建")
        else:
            save_result("数据库表结构", False, f"缺少表")
...
def verify_mock_server():
    ...
    try:
        ...
        if response.status_code == 200:
            save_result("心跳上报接口", True, f"HTTP 200")
        else:
...
def verify_pressure_routing():
    ...
    try:
        ...
        # 测试正常流量
        response = httpx.get(f"{DEMO_APP_URL}/api/users-with-header", timeout=10)
        if response.status_code == 200:
            data = response.json()
            if data:
        ...
        response = httpx.get(f"{DEMO_APP_URL}/api/users-with-header", headers=headers, timeout=10)
        if response.status_code == 200:
            data = response.json()
            if data:
...
def print_report():
    ...
    if passed == total:
        log("  所有验证通过!")
        log("\n下一步操作:")
        log("  1. 查看心跳记录：mysql -u root -p123456 -e 'SELECT * FROM trodb.t_agent_report'")
        log("  2. 测试压测流量：curl http://localhost:8000/api/users-with-header -H 'x-pressure-test: true'")
    else:
